...
def get_dict(text):
    ...
    for key in prompt_dict.keys():
        if key in text:
            ...
            text = text.replace(
                key, ""
            )  # 从长倒短查找文本中含有的字典原文，找到后就删除它，避免出现长字典包含短字典的情况
        if text == "":
            break
    return res
            ...
            with concurrent.futures.ThreadPoolExecutor(
                ...
                for future in concurrent.futures.as_completed(future_to_trans):
                    try:
                        ...
                        translations = response_json.get("message", {}).get("content")
                        print(f"{prompt}\n{translations}")
                        ...
                        if has_special_start and has_special_end:
                            ...
                            elif has_special_start and not translations.startswith(
                                special_char_start
                            ):
                                translations = special_char_start + translations
                            elif has_special_end and not translations.endswith(
                                special_char_end
                            ):
                                translations = translations + special_char_end
                        ...
                        if any(
                            apology_phrase in translations
                            for apology_phrase in apology_phrases
                        ):
                ...
                if not contains_japanese_characters and not repeat_check:
                    ...
                    print(
                        "\033[31m检测到译文中包含日文字符，尝试使用下一个提示词进行翻译。\033[0m"
                    )
                    continue
...
def index():
    return render_template_string(
        """
        <h1>欢迎使用翻译服务</h1>
        <p>请访问 <a href="/translate?text=你的文本">http://127.0.0.1:4000/translate?text=你的文本</a> 进行翻译。</p>
        """
    )
